...
public class JDBCDriver implements Driver {
	 ...
	 * automatically so that it can be accessible through
	...
	/**
	 * This method parses the URL string to make
	 * sure it is valid. It only accepts the format:
	 * jdbc:dana:localhost:<dbname>, where dbname
	 * is optional. The method returns <dbname> if
	 * the URL is valid, or null otherwise.
	 */
	...
	/**
	 * Retrieves whether the driver thinks that it can open a
	 * connection to the given URL. Typically drivers will return
	 * true if they understand the subprotocol specified in the
	 * URL and false if they do not.
	 * Parameters:
	 ...
	 * wrong kind of driver to connect to the given URL. This will
	 * be common, as when the JDBC driver manager is asked to connect
	 * to a given URL it passes the URL to each loaded driver in turn.
	 * The driver should throw an SQLException if it is the right driver
	 ...
	 * to get enough information to connect to a database. Note that depending
