...
public class Condition {
	...
	public boolean meetsCondition(Record rec) {
		...
		for(int i = 0; i<condition.length(); i++){
			...
			String simpleCond = "";
			int bracesCounter = 0;
			while(i<condition.length() && condition.charAt(i) != '&' && condition.charAt(i) != '|'
					&& condition.charAt(i) != '!'){
				simpleCond += condition.charAt(i);
				if(condition.charAt(i) == '(')		bracesCounter++;
				if(condition.charAt(i) == ')')		if(--bracesCounter == 0)	break;
				i++;
			}

			if(simpleCond.length() != 0 && bracesCounter == 0 ){
				if(simpleCond.contains("=") || simpleCond.contains("<") || simpleCond.contains(">")){
					substitute = substitute.replaceFirst(simpleCond, 
						(new SimpleCondition(simpleCond, table).meetsCondition(rec))? "T":"F" );
				}
				else if(rec.getValue(simpleCond) instanceof Boolean){			// the cell it self might be of boolean type
	...
	private boolean valueOfExplicitExp(String exp) {
		...
		for (int i = 0; i < exp.length(); i = (increaseFlag) ? i + 1 : i) {
			...
			} else // evaluate the top operation in stack
			{
				increaseFlag = false;
				if (operations.peek() == '!')
					values.push(evaluateSimpleExp(values.pop(), operations.pop(), true));
				else if (operations.peek() == '(' && exp.charAt(i) == ')') {
					operations.pop();
					increaseFlag = true;
				} else
					values.push(evaluateSimpleExp(values.pop(), operations.pop(), values.pop()));
			}
		}
	...
	private boolean evaluateSimpleExp(boolean op1, char operator,
			boolean op2) {
		switch (operator) {
		case '&':
			return op1 & op2;
